...
public:
  ...
  Type* GetBaseAddr() const { return m_p; }

  int GetSize() const { return m_rows * m_cols; }

  int GetSizeInBytes() const { return GetSize() * sizeof(Type); }

  const Type& Get(int Col, int Row) const { return *GetAddr(Col, Row); }
  ...
  void Set(int Col, int Row, const Type& Val) { *GetAddr(Col, Row) = Val; }
