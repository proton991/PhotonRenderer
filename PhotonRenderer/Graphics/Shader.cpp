...
bool LinkProgram(GLuint id) {
  ...
  if (success == GL_FALSE) {
    ...
    glGetProgramInfoLog(id, logLen, nullptr, infoLog.data());
    LOGE("Failed to link program: {}", std::string(infoLog.begin(), infoLog.end()));
    return false;
  }
  ...
  if (success == GL_FALSE) {
    glGetProgramiv(id, GL_INFO_LOG_LENGTH, &logLen);
    std::vector<GLchar> info_log(logLen);
...
Ref<ShaderProgram> ShaderProgramFactory::CreateShaderProgram(const ShaderProgramCreateInfo& info) {
  LOGT("Building shader program {}", info.name);
  std::vector<GLuint> shaderIds;
  shaderIds.reserve(info.stages.size());
  bool success = true;
  for (const auto& stage : info.stages) {
    auto id{glCreateShader(ShaderStage::Name2GL_ENUM.at(stage.type))};
    auto shader_code = util::ShaderCache::LoadShaderSource(stage.filePath);
    if (shader_code.empty()) {
      return nullptr;
    }
    if (!CompileShader(id, shader_code)) {
      success = false;
      break;
    }
    shaderIds.emplace_back(id);
  }
  ...
  return CreateRef<ShaderProgram>(info.name, programId);
}
...
ShaderProgram& ShaderProgram::SetUniform(const std::string& name, const glm::mat3x3& value) {
  if (m_uniforms.contains(name)) {
...
ShaderProgram& ShaderProgram::SetUniform(const std::string& name, const glm::mat4x4& value) {
  if (m_uniforms.contains(name)) {
